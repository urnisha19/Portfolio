...
import echangeSite from "../../assets/Projects/Exchange Site.png";
import { Link } from 'react-router-dom';

const projects = [
  { img: echangeSite, alt: "EchangeSite Project" },
  { img: eventpick, alt: "EventPick Project" },
...
const MyWork = () => {
  return (
    <div className="mx-4 md:mx-16">
      <h2 className="text-3xl font-bold mb-8 text-center text-rebeccapurple">Featured Projects</h2>
      <div className="carousel carousel-center rounded-box bg-lavender p-5 space-x-4 overflow-x-auto scrollbar-hide">
        {projects.map(({ img, alt }, idx) => (
          <div key={idx} className="carousel-item flex-shrink-0 rounded-lg shadow-md hover:shadow-xl transition-shadow cursor-pointer">
            <img src={img} alt={alt} className="w-60 h-72 object-cover rounded-lg" loading="lazy" draggable={false} />
          </div>
        ))}
      </div>
      <Link
